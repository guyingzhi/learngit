#include "encode_AFN0F.h"

int encode_AFN0F(str_encode_param *astr_param, uchar *usr_dat)
{
	int li_i, li_j;    
	int ret=0, li_len=0;
	...
	if (astr_param->mul_pn.cnt != 1)
		return ERROR_RANGE_PN;
		...
		for (li_j = 0; li_j < astr_param->mul_fn.cnt; li_j++){
			...
			*p = data_id.DT2;
			...
			switch (astr_param->mul_fn.fn[li_j]){
				case 1:
					ret = en_AFN0F_FN1(astr_param->dat, 99, p);  /*无数据内容*/
					break;
				default :
					p -= 4;
					li_len -= 4;
