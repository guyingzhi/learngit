#include "decode_AFN03.h"

int decode_AFN03(str_encode_param *astr_frame)
{
	uchar *p;
 ...
 /*   li_res = get_zd_gylx(astr_frame->addr, &(astr_frame->gylx));
	...
	while (li_len > 0){
		...
		if (lstr_fn.cnt != 1)
			return ERROR_RANGE_DATA;
		if (lstr_fn.fn[0] > 4 || lstr_fn.fn[0] == 0)
			return ERROR_DATA_INVALID;
		p += 4;
		li_len -= 4;
		li_step = 0;
		switch (lstr_fn.fn[0]){
			case 1:   /*中继站工作状态控制*/
			case 2:   /*中继站工作状态查询*/
				li_res = de_AFN03_FN1_2(p, astr_frame, lstr_pn.pn[0], lstr_fn.fn[0], &li_step, li_len);
			...
			case 3:  /*中继站工作状态切换记录查询*/
				li_res = de_AFN03_FN3(p, astr_frame, lstr_pn.pn[0], lstr_fn.fn[0], &li_step, li_len);
				if (li_res == ERROR_DATA_INVALID || li_res == ERROR_FRAME_DATA_LENGTH)
					return li_res;
				p += li_step;
				li_len -= li_step;	
			...
			case 4:	 /*中继站运行状况统计数据查询*/
				li_res = de_AFN03_FN4(p, astr_frame, lstr_pn.pn[0], lstr_fn.fn[0], &li_step, li_len);
				if (li_res == ERROR_DATA_INVALID || li_res == ERROR_FRAME_DATA_LENGTH)
					return li_res;
				p += li_step;
				li_len -= li_step;
