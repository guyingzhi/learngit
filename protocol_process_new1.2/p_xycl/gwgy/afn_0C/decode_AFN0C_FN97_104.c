#include "decode_AFN0C_FN97_104.h"
...
int decode_afn0C_F97_F99(uchar *as_data, str_AFN0C0D_QX *astr_val, int *ai_step)
{
	...
	{
		astr_val->isnull[lc_i] = getdataformat07(p, &(astr_val->val[lc_i]), &li_step);
...
int decode_afn0C_F100_F103(uchar *as_data, str_AFN0C0D_QX *astr_val, int *ai_step)
{
	...
	for (lc_i = 0; lc_i < astr_val->val_num; lc_i++)
	{
		astr_val->isnull[lc_i] = getdataformat25(p, &(astr_val->val[lc_i]), &li_step);
		astr_val->val_time[lc_i] = ldt_dt;
		after_time(&ldt_dt, &ldt_dt, 60 * lc_permin);
		*ai_step += li_step;
		p += li_step;
